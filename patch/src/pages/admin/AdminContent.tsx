...
import { Edit, Save, Plus, Trash2, Upload, X, RefreshCw, Loader2, Image as ImageIcon } from 'lucide-react'
...
function ImageUploader({ bucket = 'site-images', onUploaded }: { bucket?: string; onUploaded: (url: string) => void }) {
  ...
  return (
    ...
    >
      ...
      {uploading
        ...
        : <Upload size={20} className="mx-auto mb-2 text-white/25" />}
      <p className="text-white/30 text-sm">{uploading ? 'Uploading…' : 'Click or drag an image to upload'}</p>
    </div>
  )
}
...
export default function AdminContent() {
  ...
  const [newMedia,     setNewMedia]     = useState({ title: '', content: '', type: 'article', image_url: '' })
  ...
  const saveMedia = async (e: React.FormEvent) => {
    ...
    try {
      const p = { title: newMedia.title, content: newMedia.content, type: newMedia.type, image_url: newMedia.image_url || null, published: true }
      if (editingMedia) {
      ...
      setNewMedia({ title: '', content: '', type: 'article', image_url: '' })
      loadMedia()
    } catch (err: any) { toast.error(err.message) }
  ...
  const tabs = [
    { key: 'about'    as Tab, label: 'About Page', icon: '🏛️' },
    { key: 'events'   as Tab, label: 'Events',     icon: '📅' },
    { key: 'media'    as Tab, label: 'Media',      icon: '📸' },
    { key: 'programs' as Tab, label: 'Programs',   icon: '🎓' },
  ]
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          {tab === 'about' && (
            <div>
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-xl font-bold">🏛️ About Page</h2>
                  <p className="text-white/35 text-sm">Edit text on the public About page — supports bold, italic, alignment &amp; lists</p>
                </div>
                ...
                <div className="space-y-4">
                  {aboutFields.map(field => (
                    <div key={field.key} className="glass rounded-2xl p-6">
                      ...
                      {/* FIX: use RichEditor instead of plain textarea */}
                      {editingId === field.key
          ...
          {tab === 'events' && (
            <div>
              ...
              {showAddEvent && (
                <form onSubmit={saveEvent} className="glass rounded-2xl p-6 mb-5 border border-ysdi/20 space-y-4">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer text-sm text-white/60"><input type="checkbox" checked={newEvent.virtual} onChange={e => setNewEvent(n => ({ ...n, virtual: e.target.checked }))} className="accent-purple-500 w-4 h-4" /> Virtual</label>
                  </div>
              ...
              {loadingEvents ? <Spinner /> : events.length === 0
                ? <div className="text-center py-16 glass rounded-2xl"><div className="text-4xl mb-3">📅</div><p className="text-white/40">No events yet.</p></div>
          ...
          {tab === 'media' && (
            <div>
              <div className="flex items-center justify-between mb-6">
                <div><h2 className="text-xl font-bold">📸 Media Posts</h2><p className="text-white/35 text-sm">Articles, videos, galleries, flyers — with image upload</p></div>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { setEditingMedia(null); setNewMedia({ title: '', content: '', type: 'article', image_url: '' }); setShowAddMedia(true) }} className="btn-ysdi text-sm"><Plus size={15} /> Add Post</button>
                </div>
              </div>
              {showAddMedia && (
                <form onSubmit={saveMedia} className="glass rounded-2xl p-6 mb-5 border border-ysdi/20 space-y-4">
                  ...
                  <div><label className="label">Type</label><select className="input-glass w-full" style={ss} value={newMedia.type} onChange={e => setNewMedia(n => ({ ...n, type: e.target.value }))}><option style={ss}>article</option><option style={ss}>video</option><option style={ss}>gallery</option><option style={ss}>flyer</option></select></div>
                  <div>
                    <label className="label">Content</label>
                    {/* FIX: RichEditor with full formatting support */}
                    <RichEditor value={newMedia.content} onChange={v => setNewMedia(n => ({ ...n, content: v }))} rows={6} />
                  </div>
                  {/* FIX: real image upload via Supabase Storage */}
                  <div>
                    <label className="label flex items-center gap-2"><ImageIcon size={13} /> Cover Image</label>
                    {newMedia.image_url
                      ? <div className="relative inline-block"><img src={newMedia.image_url} alt="cover" className="h-28 rounded-xl object-cover" /><button type="button" onClick={() => setNewMedia(n => ({ ...n, image_url: '' }))} className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 rounded-full flex items-center justify-center"><X size={12} /></button></div>
                      : <ImageUploader onUploaded={url => setNewMedia(n => ({ ...n, image_url: url }))} />}
                  </div>
                  <div className="flex gap-3">
              ...
              {loadingMedia ? <Spinner /> : media.length === 0
                ? <div className="text-center py-16 glass rounded-2xl"><div className="text-4xl mb-3">📸</div><p className="text-white/40">No posts yet.</p></div>
                : <div className="space-y-3">{media.map(post => (
                    <div key={post.id} className="glass rounded-2xl p-5 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-4 min-w-0">
                        {post.image_url && <img src={post.image_url} alt={post.title} className="w-14 h-14 rounded-xl object-cover flex-shrink-0" />}
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className="text-xs bg-emerald-400/10 text-emerald-400 px-2 py-0.5 rounded-full">Live</span>
                        <button onClick={() => { setEditingMedia(post); setNewMedia({ title: post.title, content: post.content || '', type: post.type, image_url: post.image_url || '' }); setShowAddMedia(true) }} className="glass p-1.5 rounded-lg text-white/30 hover:text-white"><Edit size={13} /></button>
            ...
            <div>
              ...
              {showAddProgram && (
                <form onSubmit={saveProgram} className="glass rounded-2xl p-6 mb-5 border border-ysdi/20 space-y-4">
                  ...
                  {/* FIX: image upload for programs */}
                  <div>
                    <label className="label flex items-center gap-2"><ImageIcon size={13} /> Program Image</label>
                    {newProgram.image_url
                      ...
                      : <ImageUploader onUploaded={url => setNewProgram(n => ({ ...n, image_url: url }))} />}
                  </div>
              ...
              {loadingPrograms ? <Spinner /> : programs.length === 0
                ? <div className="text-center py-16 glass rounded-2xl"><div className="text-4xl mb-3">🎓</div><p className="text-white/40">No programs yet.</p></div>
