...
const DEFAULTS: Record<string, Record<string, string>> = {
  ...
  ysdi: {
    mission:     'To drive sustainable development across Africa through youth empowerment, innovation, and community action.',
    vision:      'An Africa where every young person has the tools, platform, and support to drive meaningful change in their community.',
    founder_bio: 'Ayotunde Aboderin is a youth advocate and social impact leader committed to expanding access to leadership development and advocacy platforms for young people across Africa.',
    about_body:  'YSDI Initiative is a Pan-African youth development organisation dedicated to empowering young leaders to drive sustainable change.',
  },
}
...
export default function About() {
  ...
  const dbDomain = isYSDI ? 'ysdi' : 'ampliyouth'
  ...
  const defaults = DEFAULTS[dbDomain]

  const [content, setContent] = useState<Record<string, string>>(defaults)

  useEffect(() => {
    // FIX: filter by domain so YSDI and AmpliYouth content don't bleed into each other
    supabase
      .from('site_content')
      .select('key, value')
      .eq('domain', dbDomain)
      .then(({ data }) => {
        if (data && data.length > 0) {
          ...
          data.forEach(row => {
            // DB keys are prefixed e.g. "ysdi_mission" → strip prefix to get "mission"
            const shortKey = row.key.replace(`${dbDomain}_`, '')
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <div className="space-y-5 text-white/55 leading-relaxed">
              <p dangerouslySetInnerHTML={{ __html: content.about_body }} />
              ...
              <p>{isYSDI ? 'YSDI Initiative' : 'AmpliYouth'} was created to bridge this gap — ensuring that talent, integrity, and purpose are not limited by geography or socioeconomic background.</p>
            </div>
          </div>
      ...
      <section className="section">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-6">
          <div className={`glass-primary p-10 rounded-3xl border ${accentBorder}`}>
            ...
            <p className="text-white/55 leading-relaxed" dangerouslySetInnerHTML={{ __html: content.mission }} />
          </div>
          <div className="glass p-10 rounded-3xl">
            ...
            <p className="text-white/55 leading-relaxed" dangerouslySetInnerHTML={{ __html: content.vision }} />
          </div>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className={`glass rounded-3xl p-10 border ${accentBorder}`}>
            ...
            <p className="text-white/55 leading-relaxed max-w-2xl mx-auto"
              dangerouslySetInnerHTML={{ __html: content.founder_bio }} />
          </div>
        </div>
      </section>
