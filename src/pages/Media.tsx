                ...
                return (
                  <div key={post.id} className="glass-hover overflow-hidden group">
                    <div className="relative h-48 overflow-hidden bg-gradient-to-br from-white/5 to-white/2 flex items-center justify-center">
                      {post.media_url
                        ? <img src={post.media_url} alt={post.title} className="w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-500"/>
                      ...
                      {post.type === 'video' && (
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="w-12 h-12 rounded-full glass flex items-center justify-center"><Play size={20} className="fill-white ml-1"/></div>
                        </div>
                      )}
                    </div>
                    <div className="p-6">
                      <h3 className="font-semibold leading-snug mb-2">{post.title}</h3>
                      {post.content && <p className="text-white/40 text-sm line-clamp-2 mb-4">{post.content}</p>}
